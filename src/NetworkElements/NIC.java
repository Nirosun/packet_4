...
public class NIC {
	...
	public void sendPacket(Packet packet, PacketConsumer parent){
		if(this.trace){
			System.out.println("Trace (NIC): Packet ready to send, delay = " + packet.getDelay());
			if(this.link==null)
	...
	public void receivePacket(Packet packet){
		if (this.buffer.size() < this.bufferSize) {		// if overflow, drop packet
			this.buffer.add(packet);
			packet.addDelay(1);
		}			
		
		if (trace) {
			System.out.println("Trace (NIC): Received packet, delay = " + packet.getDelay());			
		}
		if (this.parent instanceof Computer) {	// packet reaches destination
			// add delay and packet sent
			Metrics.totalDelay += packet.getDelay();
			Metrics.packetsSucceed ++;
			if (trace) {
				System.out.println("Packet reaches destination.");
			}
		}
	}
